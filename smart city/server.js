...
// Add these imports at the top of your server2.js file
...
const ECONOMY_API_KEYS = {
    ALPHA_VANTAGE: '', // Your existing key
    FINNHUB: '', // Get from finnhub.io
    POLYGON: '', // Get from polygon.io
    TWELVE_DATA: '', // Get from twelvedata.com
    CURRENCY_API: '' // Get from currencyapi.com
};
...
app.post('/api/chat/health', async (req, res) => {
    try {
        ...
        const systemPrompt = `You are HealthBot, an AI medical assistant for residents of Ballarpur and Chandrapur, Maharashtra, India. 

Your capabilities:
- Provide helpful, accurate medical information and health advice
- Analyze symptoms and suggest appropriate next steps
- Offer wellness tips and preventive care guidance
- Provide information about local healthcare facilities
- Give medication information and safety advice
- Offer mental health support and guidance

Important guidelines:
- Always be empathetic, supportive, and professional
- Use clear, non-technical language that everyone can understand
- Always recommend consulting healthcare professionals for serious symptoms
- Include emergency contact reminders when appropriate (Call 102 for ambulance, 108 for emergency)
- Provide personalized responses based on the user's specific situation
- Be culturally sensitive to Indian healthcare practices
- Suggest local hospitals and clinics when appropriate
- Never provide specific drug dosages - always refer to healthcare providers

Local context:
- You serve Ballarpur and Chandrapur districts in Maharashtra
- Local emergency number: 102 (ambulance), 108 (emergency services)
- Major hospitals: Government Hospital Chandrapur, Mayo Hospital Nagpur
- Common local health concerns: seasonal illnesses, air quality issues, heat-related conditions

Remember: You are not a replacement for professional medical care, but a supportive health information assistant.`;
        ...
        const response = await axios.post(
            `${NETMIND_CONFIG.BASE_URL}/chat/completions`,
            {
                model: NETMIND_CONFIG.MODEL,
                messages: messages,
                max_tokens: NETMIND_CONFIG.MAX_TOKENS,
                temperature: NETMIND_CONFIG.TEMPERATURE,
                stream: false
            },
            {
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${NETMIND_CONFIG.API_KEY}`
                },
        ...
        console.error('Health Chat API Error:', error.message);
...
🏥 Local hospitals: Government Hospital Chandrapur, Mayo Hospital Nagpur

If this isn't urgent, please try again in a few moments. I'm here to help with health information, symptom guidance, and wellness advice.

Is this a medical emergency? If so, please contact emergency services immediately.`;
...
// General Chatbot endpoint for municipal services
app.post('/api/chat/municipal', async (req, res) => {
    try {
        const { message, location = 'chandrapur', conversation_history = [] } = req.body;
        
        if (!message || !message.trim()) {
            return res.status(400).json({ error: 'Message is required' });
        }

        const locationInfo = LOCATIONS[location] || LOCATIONS['chandrapur'];

        const systemPrompt = `You are MunicipalBot, an AI assistant for ${locationInfo.name}, ${locationInfo.district} district, Maharashtra, India.

Your role:
- Help residents with municipal services and information
- Provide information about local government services
- Guide users on complaint submission and tracking
- Share information about local amenities and facilities
- Help with civic processes and documentation
- Provide local news and announcements
...
- Birth/death certificate processes
- Building permits and approvals
- Waste management and sanitation
- Public transportation information
- Local government contact information

Location context for ${locationInfo.name}:
- Population: ${locationInfo.population.toLocaleString()}
- Area: ${locationInfo.area} km²
- PIN Code: ${locationInfo.pincode}
- District: ${locationInfo.district}
- Major industries: ${location === 'ballarpur' ? 'Paper Manufacturing, Textiles' : 'Coal Mining, Power Generation, Steel'}

Guidelines:
- Be helpful and informative about local services
- Provide step-by-step guidance for municipal processes
- Always suggest contacting relevant departments for official matters
- Be encouraging about civic participation
- Suggest using the complaint system for issues
- Provide accurate local information

Remember: You're here to make municipal services more accessible to residents.`;
        ...
        const response = await axios.post(
            `${NETMIND_CONFIG.BASE_URL}/chat/completions`,
            {
                ...
                messages: messages,
                max_tokens: NETMIND_CONFIG.MAX_TOKENS,
                temperature: 0.6, // Slightly lower for more consistent municipal info
                stream: false
            },
            {
                headers: {
        ...
        console.error('Municipal Chat API Error:', error.message);
        
        const fallbackResponse = `I'm experiencing technical difficulties right now. 

For immediate municipal services:
📞 Municipal Office: Contact your local municipal corporation
🏛️ Location: ${LOCATIONS[req.body.location || 'chandrapur'].name} Municipal Office
📧 Services: Property tax, birth certificates, water supply issues

You can also:
✅ Submit complaints through our complaint system
📋 Check service request status
📢 View local announcements

Please try again in a few moments. I'm here to help with municipal services and local information.`;
...
app.post('/api/healthbot/chat', async (req, res) => {
    try {
        ...
        const systemPrompt = `You are HealthBot, an AI medical assistant. Provide helpful, accurate medical information while being empathetic and professional. Always remind users to consult healthcare professionals for serious concerns and to call emergency services for emergencies.`;
        ...
        console.error('HealthBot API error:', err.message);
        res.status(500).json({ error: 'Failed to get response from HealthBot' });
    }
});


// General purpose chatbot endpoint
app.post('/api/chat/general', async (req, res) => {
    try {
        ...
        const systemPrompt = `You are a helpful AI assistant for residents of Ballarpur and Chandrapur in Maharashtra, India. You're knowledgeable about:
...
- Technology help and guidance
- Business and economic information
- Travel and transportation
- Entertainment and lifestyle

Local context:
- You serve the Chandrapur district in Maharashtra
- You're familiar with local culture, festivals, and traditions
- You can provide information in a culturally appropriate manner
- You understand the local context and challenges
...
Remember: You're a general-purpose assistant that understands the local context and can help with a wide variety of topics.`;
        ...
        const fallbackResponse = `I'm having some technical difficulties at the moment. 

I'm here to help with:
💬 General questions and conversations
🌟 Information about Ballarpur and Chandrapur
📚 Learning and educational support
🌤️ Weather and local information
📰 News and current events

Please try your question again in a few moments. I'll do my best to provide helpful information and assistance!`;
...
app.post('/api/chat/stream', async (req, res) => {
    try {
        const { message, type = 'general', location = 'chandrapur', conversation_history = [] } = req.body;
        ...
        switch (type) {
            case 'health':
                systemPrompt = `You are HealthBot, a medical assistant for Ballarpur and Chandrapur residents...`; // Use health prompt from above
                break;
            case 'municipal':
                systemPrompt = `You are MunicipalBot for ${LOCATIONS[location]?.name || 'Chandrapur'}...`; // Use municipal prompt from above
                break;
            default:
                systemPrompt = `You are a helpful AI assistant for residents of Ballarpur and Chandrapur...`; // Use general prompt from above
        }
        ...
        console.error('Stream Chat API Error:', error.message);
        res.write(`data: ${JSON.stringify({ 
...
app.get('/api/chat/health-check', async (req, res) => {
    try {
        // Test NetMind API connectivity
        ...
        res.json({
            ...
            test_response: testResponse.data.choices[0].message.content
        });
...
app.get('/api/chat/config', (req, res) => {
    res.json({
        ...
        max_tokens: NETMIND_CONFIG.MAX_TOKENS,
        temperature: NETMIND_CONFIG.TEMPERATURE,
        available_types: ['health', 'municipal', 'general'],
        supported_locations: Object.keys(LOCATIONS),
        streaming_available: true
    });
});


// GET all anonymous complaints (with pagination + filters + sorting)
app.get('/api/comp2/anonymous', async (req, res) => {
    try {
        const { page = 0, location = '', category = '', sort = 'recent' } = req.query;
        const PAGE_SIZE = 5;
        ...
        const collection = db.collection('comp2');
        ...
        console.error('Anonymous comp2 fetch error:', error.message);
        res.status(500).json({ success: false, error: 'Failed to fetch anonymous complaints' });
    }
});


// POST new anonymous complaint
app.post('/api/comp2/anonymous', async (req, res) => {
    try {
        ...
        const result = await db.collection('comp2').insertOne(complaint);
        res.json({ success: true, complaint_id: result.insertedId });
    } catch (err) {
...
app.get('/api/comp2/anonymous/:id/comments', async (req, res) => {
    try {
        const comments = await db.collection('comments')
            .find({ complaintId: new ObjectId(req.params.id) })
            .sort({ created_at: -1 })
            .toArray();
...
app.post('/api/comp2/anonymous/:id/comments', async (req, res) => {
    try {
        const { content } = req.body;
        if (!content) return res.status(400).json({ success: false, error: 'Comment required' });

        const comment = {
            complaintId: new ObjectId(req.params.id),
            content,
            created_at: new Date()
        };

        await db.collection('comments').insertOne(comment);
        await db.collection('comp2').updateOne(
